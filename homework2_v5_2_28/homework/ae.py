...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            ...
            self.proj = torch.nn.Linear(latent_dim, bottleneck) if latent_dim != bottleneck else None
    ...
    class PatchDecoder(torch.nn.Module):
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # raise NotImplementedError()
            self.proj = torch.nn.Linear(bottleneck, latent_dim) if latent_dim != bottleneck else None
            self.activation = torch.nn.GELU()
